...
class HGCN(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, rel_names, num_classes, n_steps, n_etypes):
        super(HGCN, self).__init__()
        # self.conv1 = HeteroGraphConv({rel: GraphConv(in_feats, hid_feats) for rel in rel_names}, aggregate='sum')
        self.gatedconv1 = HeteroGraphConv({rel: GatedGraphConv(in_feats, hid_feats, n_steps, n_etypes) for rel in rel_names}, aggregate='sum')
        self.gatconv = HeteroGraphConv({rel: GATConv(hid_feats, hid_feats, num_heads=3) for rel in rel_names}, aggregate='sum')
        self.gatedconv2 = HeteroGraphConv({rel: GatedGraphConv(hid_feats, out_feats, n_steps, n_etypes) for rel in rel_names}, aggregate='sum')
        self.classify = nn.Linear(out_feats, num_classes)
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


    def forward(self, graph, inputs, e_type):
        # inputs是节点的特征
        h = self.gatedconv1(graph, inputs, e_type)
        h = {k: F.normalize(v) for k, v in h.items()}
        h = self.gatconv(graph, h)
        h = {k: F.relu(v) for k, v in h.items()}
        h = {k: F.dropout(v, 0.3) for k, v in h.items()}
        h = self.gatedconv2(graph, h, e_type)
...
class SiameseGCN(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_feats, rel_names, n_classes, n_steps, n_etypes):
        super(SiameseGCN, self).__init__()
        self.hetegcn = HGCN(in_dim, hidden_dim, out_feats, rel_names, n_classes, n_steps, n_etypes)

    def forward(self, g1, g2):
        # (32,1,2)
        h1 = self.hetegcn(g1, g1.ndata['hy'], g1.edata['ac'])
        h2 = self.hetegcn(g2, g2.ndata['hy'], g1.edata['ac'])
...
class TrainTest:
    def __init__(self, train_dataloader, validation_dataloader, test_dataloader):
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = SiameseGCN(self.dim_nfeats, self.hidden1_feats, self.hidden2_feats, self.etypes, self.gclasses, self.n_steps, self.n_etypes).to(self.device)
    ...
    def trainmodel(self):
        for epoch in range(self.batchepoch):
            ...
            for batched_graph, labels in self.train_dataloader:
                # efeats1 = batched_graph[0].etypes
                # efeats1 = torch.tensor(list(map(int, efeats1)))
                # efeats2 = batched_graph[1].etypes
                # efeats2 = torch.tensor(list(map(int, efeats2)))
